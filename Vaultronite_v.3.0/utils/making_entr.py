...
from utils.handler import add_entry_to_db, get_data


class MakeEntr(ct.CTkToplevel):
    name_error = False
    noname_error = False
    ...
    def build_ui(self, db_file, vmp, vname, vid, parent) -> None:
        ...
        # Username
        self.uname_frame, self.uname_label, self.uname_entry = create_entry(
            self, "Type Username: "
        )
        ...
        # For Vault Buttons
        self.ebtn_frame = ct.CTkFrame(self, fg_color="transparent")
        self.ebtn_create_entry = ct.CTkButton(
            self.ebtn_frame,
            text="Create Entry",
    ...
    def on_click_create_entry(self, db_file, vmp, vid, vname, parent) -> None:
        ...
        Args:
            ...
            vname (str): The vault name.
            vid (int): The vault id.
            parent (ct.CTk): The parent widget for the GUI-based application.
        ...
        if username == "":
            ...
            if not self.username_error:
                ...
                self.username_err_frame, self.username_err_label = show_error(
                    self, f"Username is invalid. Please verify"
                )
        else:
            # Remove the error message if it was previously shown and the name is not empty
            if self.username_error:
                self.win_height -= 50
                self.username_err_frame.destroy()
        ...
        if not any(
            ...
            add_entry_to_db(db_file, vmp, vid, sname, pwd, username)
            self.destroy()
