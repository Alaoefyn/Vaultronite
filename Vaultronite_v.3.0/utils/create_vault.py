...
from utils.handler import add_vault_to_db, get_data


class CreateVault(ct.CTkToplevel):
    name_error = False
    noname_error = False
    ...
    def __init__(self, db_file: str, parent: ct.CTk, *args, **kwargs):
        ...
        self.title("Please Create Your New Vault")  # title
        self.geometry(f"{self.win_width}x{self.win_height}")  # dimensions

        self.build_ui(db_file, parent)

    def build_ui(self, db_file: str, parent: ct.CTk):
        """Initialize the widgets necessary for functioning of the add vault dialog.

        Args:
            db_file (str): The database file name.
            parent (ct.CTk): The parent widget for the add vault dialog.

        Returns:
            None
        """

        def create_entry(parent: ct.CTkToplevel, label_text: str, for_pwd=False):
            """Create a label-entry pair for the add vault dialog.

            Args:
                parent (ct.CTk): The parent widget for the label-entry pair.
            ...
            frame = ct.CTkFrame(parent, fg_color="transparent")
            label = ct.CTkLabel(frame, text=label_text)
            entry = ct.CTkEntry(frame, show="●") if for_pwd else ct.CTkEntry(frame)
        ...
        # For Vault Buttons
        self.vbtn_frame = ct.CTkFrame(self, fg_color="transparent")
        self.vbtn_create_vault = ct.CTkButton(
            self.vbtn_frame,
    ...
    def on_click_create_vault(self, db_file: str, parent: ct.CTk) -> None:
        """
        ...
        def show_error(parent: ct.CTkToplevel, text: str):
            """
            Displays an error message in a new window.

            Args:
                parent (ct.CTkToplevel): The parent widget for the error message window.
            ...
            """
            frame = ct.CTkFrame(parent, fg_color="#141212", corner_radius=10)
        ...
        if vname == "":
            ...
            if not self.noname_error:
                ...
                self.noname_err_frame, self.noname_err_label = show_error(
                    self, f"You Did Not Gave the Vault Name:Please Give Your Vault a Name (Ex.My Work Passwords)"
                )
        else:
        ...
        if any(ch in self.PROHIBITED_PWD_CHARS for ch in pwd):
            ...
            if not self.pwd_error:
                ...
                self.pwd_err_frame, self.pwd_err_label = show_error(
                    self,
                    f"Invalid Master Password: vault passwords cannot contain spaces or special characters like {self.PROHIBITED_PWD_CHARS}",
                )
        else:
